...
バージョン1.7.190901リリース(各種修正）\n
　
  各種修正多数\n
  POTモードの追加（画面オン時には常にTOPに本アプリがいる、PTTでの画面オンはしない）\n
  ユーザ補助の利用はオプションとする（機能面での差はほぼない）\n
　ロック画面上にいる場合といない場合で設定画面の動作を変更\n
 
バージョン1.5.190812リリース（Android9対応版）

　本版では画面オフ時にPTTを押しても本アプリは起動しない　｛デグレード｝
 （端末側の呼び出し動作に課題があるため）※ファーム挙動が改善されたら元に戻します
 
 機能の追加は無し

バージョン1.4.190304リリース（暫定）

　アプリ画面の表示FONTスケールを最大（x1.30）値に固定する修正
 　（端末設定のフォントが最小指定だと、アプリ画面がスカスカで見栄えが悪いので、スケールを固定する⇒暫定処理）
　非推奨になった設定方法なので、次で推奨関数に置き換える
　端末指定サイズのまま使いたい方は1.4.190301のままにして下さい
...
バージョン1.3.190210リリース：
　
 回線検出不良の修正、全回線切断時の表示不良修正、回線表示スタイルの修正

バージョン1.2.190217リリース：　

　Bluetooth、USBテザリング、VPNに対応　※あくまで本アプリのPTTではWIFIテザリングのみを制御します
...
本アプリケーションを利用した事によるいかなる損害に対し作者は一切の責任および負債を負いません。
...
だんだん飽きてきて、日本語がおかしいwww

好きに改造して良いATOMライフをおくっていただけたら幸いです。　　疲れた～
