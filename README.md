...
BACKボタンで再度アプリ画面に戻るとPttボタン色が黄色に変化します（グレーのままの場合は設定変更ができていません）

３．パワーセーブのホワイトリストに追加

本端末はインストール直後の初期状態はスリープするとアプリケーションがOSにより動作無効にされます（※２の設定などが元に戻される）
アプリ動作が無効にされると機能しませんので、このパワーセーブから除外してやる必要があります

アプリから抜けて端末設定を起動して下さい
...
PTTボタンは画面消灯時にはアプリ側から見えなくなります。これでは画面オフ時にPTTボタンを押しても何も起こりません。

これを解決する為にUnihertz ATOMのショートカット機能を使います。本機能を設定すればPTTの操作が画面オフ時にも検知できるようになります。

※アプリ画面を全面に出すトリガーとして本機能を使いますので、ショートカットが設定されなければ使い勝手が悪くなります
